...
class ParserTest(unittest.TestCase):
    def test_needs_operation_no_tokens(self):
        ...
        tokens = []
        self.assertFalse(parser._ExpressionParser__needs_operation(tokens))
    def test_needs_operation_after_number(self):
        parser = ExpressionParser(create_fd())
        tokens = ['5']
        self.assertTrue(parser._ExpressionParser__needs_operation(tokens))
    ...
    def test_process_number_first_token(self):
        parser = ExpressionParser(create_fd())
        tokens = []
        parser._ExpressionParser__process_number(tokens, '123')
    ...
    def test_process_number_single_decimal_point(self):
        ...
        with self.assertRaises(IsolatedDecimalPointError):
            parser._ExpressionParser__process_number(tokens, '.')
    
    def test_process_text_first_token(self):
        parser = ExpressionParser(create_fd())
        tokens = []
        parser._ExpressionParser__process_text(tokens, 'a', '*')
    ...
    def test_process_text_multiple_vars(self):
        parser = ExpressionParser(create_fd())
        tokens = []
        parser._ExpressionParser__process_text(tokens, 'abcdefg', '*')
    ...
    def test_process_text_long_vars(self):
        ...
        tokens = ['5']
        parser._ExpressionParser__process_text(tokens, 'apidpiefg', '*')
        self.assertListEqual(tokens, ['5', ' ', 'a', ' ', 'pi', ' ', 'd', ' ', 'pie', ' ', 'f', ' ', 'g'])
    def test_process_text_function(self):
        parser = ExpressionParser(create_fd())
        tokens = []
        parser._ExpressionParser__process_text(tokens, 'sin', '(')
        self.assertListEqual(tokens, ['sin', '()'])
    def test_process_text_vars_and_function(self):
        parser = ExpressionParser(create_fd())
        tokens = ['5']
        parser._ExpressionParser__process_text(tokens, 'cosin', '(')
    ...
    def test_process_text_unknown_variable(self):
        ...
        with self.assertRaises(UnknownVariableError):
            parser._ExpressionParser__process_text(tokens, 'p', '*')

    def test_process_operator_first_token(self):
        parser = ExpressionParser(create_fd())
        tokens = []
        parser._ExpressionParser__process_operator(tokens, '+')
    ...
    def test_process_left_bracket_first(self):
        ...
        tokens = []
        brackets = []
        parser._ExpressionParser__process_left_bracket(tokens, brackets, '(')
    ...
    def test_process_right_bracket_first(self):
        ...
        with self.assertRaises(BracketsMismatchError):
            parser._ExpressionParser__process_right_bracket(tokens, brackets, ')')
